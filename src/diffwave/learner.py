...
class DiffWaveLearner:
    ...
    def train_step(self, features):
        ...
        with self.autocast:
            t = torch.randint(0, len(self.params.noise_schedule), [
                              N], device=audio.device)
            noise_scale = self.noise_level[t].unsqueeze(1)
            noise_scale_sqrt = noise_scale**0.5
            noise = torch.randn_like(audio)
            noisy_audio = noise_scale_sqrt * audio + \
    ...
    def _write_summary(self, step, features, loss):
        ...
        writer.add_audio(
            'feature/audio', features['audio'][0], step, sample_rate=self.params.sample_rate)
...
def _train_impl(replica_id, model, dataset, args, params):
    ...
    learner = DiffWaveLearner(args.model_dir, model,
                              dataset, opt, params, fp16=args.fp16)
...
def train_distributed(replica_id, replica_count, port, args, params):
    ...
    _train_impl(replica_id, model, dataset_from_path(
        args.data_dirs, params, is_distributed=True), args, params)
