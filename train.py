...
from ignite.handlers import Checkpoint, EMAHandler, DiskSaver
...
def create_trainer(model, mel_spec, noise_scheduler, optimizer, criterion, scheduler, device, trainer_config, model_name: str, checkpoint_path: str):
    ...
    common.setup_common_training_handlers(
        ...
        save_handler=DiskSaver(save_dir, require_empty=False),
...
def training(config: dict):
    ...
    if device_ids:
        ...
        mel_spec = nn.DataParallel(mel_spec, device_ids=device_ids)

    trainer, ema_model = create_trainer(model, mel_spec, noise_scheduler, optimizer,
                                        criterion, scheduler, device, trainer_config,
